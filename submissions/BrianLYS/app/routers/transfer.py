...
from app.schemas import TransferResult, ErrorResponse
...
def transfer_funds(
    ...
    if req.fromAccount in store.inactive_accounts or req.toAccount in store.inactive_accounts:
    ...
    if store.accounts[req.fromAccount] < req.amount:
        ...
        store.transactions.insert(0, {"id": tx_id, "result": result, "ts": datetime.now(timezone.utc).isoformat()})
